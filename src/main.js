 ...
 * Version: 1.3 - TensorFlow.js Handpose + Three.js Metaballs
 */
...
async function detectHands() {
  ...
  try {
    ...
    if (predictions.length > 0) {
      ...
      if (handX) handX.textContent = Math.round(handTracking.lastX * 100) + '%';
      if (handY) handY.textContent = Math.round(handTracking.lastY * 100) + '%';
      if (handState) {
...
async function startCamera() {
  ...
  try {
    ...
    showCameraError('Camera: ' + error.message);
    cameraToggle.checked = false;
