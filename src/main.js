    ...
    function detectLoop() {
      ...
      if (videoElement.readyState >= 2) {
        ...
        if (result) {
          ...
          handTracking.x = (1 - result.x) * 2 - 1;
          handTracking.y = -(result.y * 2 - 1);
          handTracking.gesture = result.gesture;
          
          const handX = document.getElementById('handX');
          const handY = document.getElementById('handY');
          if (handX) handX.textContent = ((1 - result.x) * 100).toFixed(0) + '%';
          if (handY) handY.textContent = (result.y * 100).toFixed(0) + '%';
          if (handState) {
            handState.textContent = 'Detected: ' + result.gesture;
            handState.style.color = '#0ff';
          }
          
          debugLog('info', `Hand: ${result.gesture} at ${((1-result.x)*100).toFixed(0)}%, ${(result.y*100).toFixed(0)}%`);
        } else {
